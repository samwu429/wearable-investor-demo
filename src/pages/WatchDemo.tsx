  ...
  return (
    <div className="mx-auto max-w-6xl px-4 py-10 md:px-6 md:py-14">
      <div className="max-w-2xl">
        <h1 className="font-display text-3xl font-bold text-white md:text-4xl">Part 1 · 手表中枢演示</h1>
      ...
      <div className="mt-12 flex flex-col items-center gap-12 lg:flex-row lg:items-start lg:justify-center">
        <div className="relative shrink-0">
          <div
            className="absolute -inset-6 rounded-[3rem] bg-gradient-to-br from-gold/30 via-transparent to-mint/20 blur-2xl"
            aria-hidden
          />
          <motion.div
            className="relative flex h-[380px] w-[380px] items-center justify-center rounded-[3rem] border border-white/10 bg-gradient-to-b from-slate to-night p-3 shadow-2xl shadow-black/60 md:h-[420px] md:w-[420px]"
            layout
          >
            <div className="absolute inset-3 rounded-[2.6rem] border border-white/5 bg-black/40" />
            ...
            <div className="relative z-10 flex h-[300px] w-[300px] flex-col items-center justify-between rounded-full border border-white/10 bg-gradient-to-b from-[#1a2336] to-[#0b1020] p-6 shadow-inner md:h-[320px] md:w-[320px]">
              <div className="flex w-full items-start justify-between gap-2">
                <span className="rounded-full bg-white/5 px-2 py-0.5 text-[10px] text-mist ring-1 ring-white/10">
                  Hub · SLM
                </span>
                <span
                  className={`rounded-full px-2 py-0.5 text-[10px] ring-1 ${
                    slmBusy ? 'bg-mint/20 text-mint ring-mint/30' : 'bg-white/5 text-mist ring-white/10'
                  }`}
                >
                  {slmBusy ? '编排中…' : '待机'}
                </span>
              ...
              <div className="text-center">
                <p className="font-display text-5xl font-semibold tabular-nums text-white md:text-6xl">{timeStr}</p>
              ...
              <AnimatePresence mode="wait">
                {verified ? (
                  <motion.div
                    ...
                    exit={{ opacity: 0 }}
                    className="flex items-center gap-2 rounded-full bg-mint/15 px-4 py-2 text-sm font-medium text-mint ring-1 ring-mint/40"
                  >
              ...
              <div className="flex w-full items-center justify-between text-[10px] text-mist">
                <span className="rounded-md bg-white/5 px-2 py-1 ring-1 ring-white/10">SE 锁定</span>
                <span className="rounded-md bg-white/5 px-2 py-1 ring-1 ring-white/10">Lens 已连接</span>
                <span className="rounded-md bg-white/5 px-2 py-1 ring-1 ring-white/10">Signet 已连接</span>
              </div>
            </div>
          </motion.div>
        ...
        <div className="w-full max-w-lg space-y-6">
          <div className="flex flex-wrap gap-3">
            <button
              ...
              onClick={runPeh}
              disabled={running}
              className="rounded-full bg-gold px-6 py-3 text-sm font-semibold text-night transition enabled:hover:bg-gold-dim disabled:cursor-not-allowed disabled:opacity-50"
            >
            ...
            <button
              ...
              onClick={reset}
              disabled={running}
              className="rounded-full border border-white/20 px-6 py-3 text-sm font-medium text-white transition hover:bg-white/5 disabled:opacity-40"
            >
              重置
            </button>
          </div>
                ...
                >
                  <span
                    className={`flex h-8 w-8 shrink-0 items-center justify-center rounded-full text-xs font-bold ${
                      active ? 'bg-gold text-night' : done ? 'bg-mint/30 text-mint' : 'bg-white/10 text-mist'
                    }`}
                  >
                    {i + 1}
                  </span>
