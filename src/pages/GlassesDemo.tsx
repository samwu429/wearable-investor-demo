        ...
        <div
          className={
            variant === 'paper'
              ...
              : 'flex h-12 shrink-0 items-center gap-3 border-b border-white/10 bg-stone-950/25 px-3 backdrop-blur-sm'
          }
        >
          ...
          <p
            className={
              variant === 'paper'
                ...
                : 'shrink-0 border-b border-white/8 bg-stone-950/20 px-4 py-2 font-jit-mono text-[10px] text-stone-400 backdrop-blur-lg'
            }
          >
          ...
          <div
            className={
              variant === 'paper'
                ...
                : 'shrink-0 border-t border-white/10 bg-stone-950/25 px-4 py-2 font-jit-mono text-[10px] text-stone-400 backdrop-blur-sm'
            }
          >
...
function BigMapCanvas({ data }: { data: InstantMapUi }) {
  ...
  return (
    <div className="relative min-h-[220px] flex-1 bg-stone-950/18 backdrop-blur-sm md:min-h-[280px]">
      <svg className="absolute inset-0 h-full w-full opacity-95" viewBox="0 0 400 220" preserveAspectRatio="xMidYMid slice" aria-hidden>
        ...
        <rect width="100%" height="100%" fill="rgba(12, 22, 20, 0.2)" />
      ...
      <div className="absolute left-3 top-3 max-w-[min(100%,280px)] border border-teal-400/35 bg-stone-950/22 px-2.5 py-1.5 font-jit-mono text-[10px] leading-snug text-teal-50 shadow-[0_0_24px_rgba(45,212,191,0.06)] backdrop-blur-lg">
...
function JitMapPage({ preview }: { preview: InstantAppPreview }) {
  ...
  return (
    <div className="relative z-[1] flex h-full min-h-[320px] flex-col md:flex-row">
      <BigMapCanvas data={m} />
      <aside className="flex w-full shrink-0 flex-col gap-0 border-t border-white/10 bg-stone-950/20 p-0 backdrop-blur-sm md:w-[300px] md:border-l md:border-t-0">
        ...
        <div className="space-y-3 p-4">
          ...
          <div className="space-y-1.5 border border-white/10 bg-stone-950/20 p-3 font-jit-mono text-[11px] text-stone-300 backdrop-blur-sm">
            <p>
          ...
          <div className="border border-dashed border-amber-400/25 bg-stone-950/16 p-3 text-[11px] leading-relaxed text-stone-400 backdrop-blur-sm">
...
function JitChatAppPage({ preview }: { preview: InstantAppPreview }) {
  ...
  return (
    <div className="relative z-[1] flex h-full min-h-[360px] flex-col bg-transparent">
      <header className="flex items-center gap-3 border-b border-white/10 bg-stone-950/25 px-4 py-3 backdrop-blur-sm">
      ...
      <div className="min-h-0 flex-1 space-y-3 overflow-y-auto bg-stone-950/18 px-3 py-4 backdrop-blur-md">
        {c.bubbles.map((b, i) => (
          <div key={i} className={`flex ${b.role === 'me' ? 'justify-end' : 'justify-start'}`}>
            <div
              className={
                b.role === 'me'
                  ? 'max-w-[88%] border border-teal-400/35 bg-teal-950/22 px-3 py-2 font-jit-mono text-[12px] leading-relaxed text-teal-50 backdrop-blur-md'
                  : 'max-w-[88%] border border-white/12 bg-stone-950/22 px-3 py-2 font-jit-mono text-[12px] leading-relaxed text-stone-200 backdrop-blur-md'
              }
            >
      ...
      <div className="border-t border-white/10 bg-stone-950/25 px-3 py-2.5 backdrop-blur-sm">
        <div className="flex items-center gap-2 border border-white/12 bg-stone-950/18 px-3 py-2 font-jit-mono text-[11px] text-stone-400 backdrop-blur-md">
  ...
  return (
    <div className="relative z-[1] grid min-h-[400px] grid-cols-1 divide-y divide-amber-500/10 md:grid-cols-2 md:divide-x md:divide-y-0">
      <div className="flex min-h-[280px] flex-col bg-transparent md:min-h-0">
        <div className="shrink-0 border-b border-white/10 bg-stone-950/25 px-3 py-2 font-jit-mono text-[9px] uppercase tracking-[0.25em] text-amber-200/55 backdrop-blur-sm">
        ...
        <div className="min-h-0 flex-1 space-y-2 overflow-y-auto bg-stone-950/18 px-3 py-3 backdrop-blur-md">
          {c.bubbles.map((b, i) => (
            <div key={i} className={`flex ${b.role === 'me' ? 'justify-end' : 'justify-start'}`}>
              <div
                className={
                  b.role === 'me'
                    ? 'max-w-[90%] border border-teal-400/30 bg-teal-950/22 px-2.5 py-1.5 font-jit-mono text-[11px] text-teal-50 backdrop-blur-sm'
                    : 'max-w-[90%] border border-white/12 bg-stone-950/22 px-2.5 py-1.5 font-jit-mono text-[11px] text-stone-200 backdrop-blur-md'
                }
              >
      ...
      <div className="flex min-h-[280px] flex-col md:min-h-0">
        ...
        <div className="flex min-h-0 flex-1 flex-col md:flex-row">
          <BigMapCanvas data={m} />
          <aside className="flex w-full shrink-0 flex-col justify-center gap-2 border-t border-white/10 bg-stone-950/20 p-3 font-jit-mono text-[11px] text-stone-300 backdrop-blur-sm md:w-[188px] md:border-l md:border-t-0">
...
const toneClass: Record<string, string> = {
  gold: 'border-amber-500/40 bg-amber-950/24 text-amber-100 backdrop-blur-sm',
  mint: 'border-teal-500/40 bg-teal-950/22 text-teal-50 backdrop-blur-sm',
  rose: 'border-rose-500/40 bg-rose-950/24 text-rose-100 backdrop-blur-sm',
}
...
function JitScaffoldPage({ preview }: { preview: InstantAppPreview }) {
  ...
  return (
    <div className="relative z-[1] flex min-h-[380px] flex-col md:flex-row">
      <nav className="flex shrink-0 gap-1 border-b border-white/10 bg-stone-950/25 p-2 font-jit-mono backdrop-blur-sm md:w-52 md:flex-col md:border-b-0 md:border-r md:border-white/10">
        ...
        {screens.length ? (
          screens.map((s, i) => (
            <button
              key={s.label}
              type="button"
              className={`border px-3 py-2 text-left text-[11px] backdrop-blur-sm md:text-xs ${toneClass[s.tone] ?? 'border-white/12 bg-stone-950/25 text-stone-200'}`}
            >
      ...
      <main className="min-w-0 flex-1 bg-transparent p-4">
        ...
        <ul className="grid gap-3 sm:grid-cols-2">
          {modules.map((m) => (
            <li
              key={m.name}
              className="border border-white/10 bg-stone-950/20 p-3 shadow-[inset_0_1px_0_rgba(255,255,255,0.04)] backdrop-blur-sm"
            >
        ...
        {preview.codeSnippet ? (
          <pre className="mt-4 overflow-x-auto border border-teal-500/25 bg-stone-950/25 p-3 font-jit-mono text-[10px] leading-relaxed text-teal-100/95 backdrop-blur-md">
            {preview.codeSnippet}
          </pre>
        ) : null}
      </main>
      ...
      <div className="glasses-frame-bezel glasses-frame-bezel--walk flex min-h-0 flex-1 flex-col p-2 md:p-3">
        <div className="relative flex min-h-0 flex-1 flex-col overflow-hidden rounded-2xl shadow-[inset_0_1px_0_rgba(255,255,255,0.2),0_12px_48px_rgba(28,25,23,0.18)] ring-1 ring-stone-900/10">
          ...
          <div className="relative z-10 flex min-h-0 flex-1 flex-col">
            <header className="flex shrink-0 items-start justify-between gap-3 border-b border-white/10 bg-stone-950/20 px-4 py-2.5 backdrop-blur-md md:px-6 md:py-3">
              ...
              <div className="shrink-0 rounded-xl border border-amber-200/20 bg-stone-950/25 px-3 py-2 text-right shadow-sm backdrop-blur-md md:px-4">
              ...
              <AnimatePresence>
                {showOverlay ? (
                  <motion.button
                    ...
                    animate={{ opacity: 1 }}
                    exit={{ opacity: 0 }}
                    className="absolute inset-0 z-20 cursor-default border-0 bg-black/10 backdrop-blur-none"
                    onClick={closePanel}
                  />
                ) : null}
              </AnimatePresence>
                ...
                {(panel.kind === 'jit' || panel.kind === 'legal') && (
                  ...
                  >
                    {panel.kind === 'legal' ? (
                      ...
                      >
                        <div className="relative z-[1] border-b border-white/10 bg-stone-950/20 px-4 py-2.5 font-jit-mono text-[11px] leading-relaxed text-stone-200 backdrop-blur-sm">
            ...
            <footer className="relative z-40 shrink-0 border-t border-white/10 bg-gradient-to-t from-stone-950/38 via-stone-950/18 to-transparent px-4 py-3 backdrop-blur-sm md:px-6 md:py-4">
              {toast ? (
              ...
              >
                ...
                <input
                  ...
                  className="min-h-[48px] w-full flex-1 rounded-2xl border border-stone-500/35 bg-stone-950/20 px-4 py-2.5 text-base text-stone-100 placeholder:text-stone-500 outline-none ring-offset-2 ring-offset-transparent backdrop-blur-md focus:border-amber-400/55 focus:ring-2 focus:ring-amber-400/25 disabled:opacity-50 md:min-h-[52px] md:text-lg"
                />
                <button
                  type="submit"
